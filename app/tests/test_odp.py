...
def test_query_sds_and_render_rdf(mocker):
    ...
    assert g.value(dataset, DCTERMS.title) == Literal(
        "European Union Emissions Trading System (EU ETS) data from EUTL",
        lang="en",
    )
    assert g.value(dataset, DCTERMS.description) == Literal(
    ...
    assert g.value(dataset, DCAT.theme) == URIRef(
    ...
    assert set(g.objects(dataset, DCTERMS.subject)) == {
        EUROVOC[c] for c in ["5650", "434843", "6011"]
    }
    ...
    assert g.value(contact, VCARD["organisation-name"]) == Literal(
        "European Environment Agency"
    )
    homepage = g.value(contact, FOAF.homepage)
    assert g.value(homepage, SCHEMA.url) == Literal(
        "https://www.eea.europa.eu"
    )
    assert g.value(homepage, DCTERMS.title) == Literal(
        "European Environment Agency", lang="en"
    )
    ...
    assert g.value(address, VCARD["street-address"]) == Literal(
    ...
    assert g.value(landingpage, DCTERMS.title) == Literal(
        "European Union Emissions Trading System (EU ETS) data from EUTL",
        lang="en",
    )
    ...
    assert g.value(dist_v8, ADMS.status) == EU_STATUS.DEPRECATED
    ...
    assert g.value(dist_pdf, DCTERMS.title) == Literal(
        "Technical document", lang="en"
    )
    assert g.value(dist_pdf, DCTERMS["format"]) == EU_FILE_TYPE.PDF
    assert (
        g.value(dist_pdf, DCTERMS.type)
        == EU_DISTRIBUTION_TYPE.DOWNLOADABLE_FILE
    )
    ...
    assert g.value(dist_xlsx, DCTERMS.title) == Literal(
        "Translation of activity codes", lang="en"
    )
    assert g.value(dist_xlsx, DCTERMS["format"]) == EU_FILE_TYPE.XLSX
    assert (
        g.value(dist_xlsx, DCTERMS.type)
        == EU_DISTRIBUTION_TYPE.DOWNLOADABLE_FILE
    )
    ...
    assert (
        g.value(dist_vis, DCTERMS.type) == EU_DISTRIBUTION_TYPE.VISUALIZATION
    )
